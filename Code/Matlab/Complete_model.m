...
num_points  = 2^8;                 % Number of points on scanline
...
function delay_list = delay_ref_point(f_s, p, v, R_0, n0_index, angle_deg, cordic_iter)
    ...
    for i = 1:32
        ...
        [delay(cur_index),error_prev] = increment_and_compare(delay(cur_index-1), error_prev, inc_term, cur_index);
    end
    error_prev = 0;
    for i = 1:31
        cur_index = n0_index - i;
...
function [N_next,error_next] = increment_and_compare(N_prev, error_prev, inc_term, cur_index)
    if inc_term >= 0
        sign_bit = 1;
    else
        sign_bit = -1;
    ...
    for i = 1:5
        ...
        if abs(2*a*N_prev+a^2) > abs(inc_term_w_error)
            a = a - sign_bit;
            cur_error = inc_term_w_error - (2*a*N_prev + a^2) - 33;  % Last part a correction because inc_term changes for each iteration, relationship with error not constant
            break;
        end
    end
    disp("Inc term: " + inc_term);
    %disp("Inc term with error: " + inc_term_w_error);
    %disp("Subtract: " + (2*a*N_prev+a^2) + " (a = " + a + ")");
    disp("Error: " + cur_error);
    %disp(a);
...
function delay_array = delay_scanline(R_0, f_s, v, n, p, num_points,delay, cordic_iter, angle_deg)
    ...
    for k = 2:num_points
        inc_terms = 2*k + 1 + B_n;
        [scanline_delays(k,:),error_prev] = increment_and_compare_array(scanline_delays(k-1,:), error_prev, inc_terms);
    end
...
function [N_next_array,error_next_array] = increment_and_compare_array(N_prev_array, error_prev_array, inc_terms_array)
    N_next_array_cur = zeros(length(N_prev_array));
    error_next_array_cur = zeros(length(N_prev_array));
    for i = 1:length(N_prev_array)
        [N_next_array_cur(i),error_next_array_cur(i)] = increment_and_compare(N_prev_array(i), error_prev_array(i), inc_terms_array(i));
    end
